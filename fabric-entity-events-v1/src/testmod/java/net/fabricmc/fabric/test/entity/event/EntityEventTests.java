...
public final class EntityEventTests implements ModInitializer {
	...
	public static final ResourceKey<Block> TEST_BED_KEY = ResourceKey.create(
			Registries.BLOCK,
			ResourceLocation.fromNamespaceAndPath("fabric-entity-events-v1-testmod", "test_bed")
	);
	public static final Block TEST_BED = new TestBedBlock(BlockBehaviour.Properties.of().strength(1, 1).setId(TEST_BED_KEY));
	public static final ResourceKey<Item> DIAMOND_ELYTRA_KEY = ResourceKey.create(Registries.ITEM, ResourceLocation.fromNamespaceAndPath("fabric-entity-events-v1-testmod", "diamond_elytra"));
	public static final Item DIAMOND_ELYTRA = new Item(new Item.Properties().component(DataComponents.GLIDER, Unit.INSTANCE).equippable(EquipmentSlot.CHEST).setId(DIAMOND_ELYTRA_KEY));

	@Override
	public void onInitialize() {
		Registry.register(BuiltInRegistries.BLOCK, TEST_BED_KEY, TEST_BED);
		Registry.register(BuiltInRegistries.ITEM, TEST_BED_KEY.location(), new BlockItem(TEST_BED, new Item.Properties().setId(ResourceKey.create(Registries.ITEM, TEST_BED_KEY.location()))));
		...
		EntitySleepEvents.ALLOW_SLEEP_TIME.register((player, sleepingPos, vanillaResult) -> {
			...
			if (player.level().isBrightOutside() && player.getItemInHand(InteractionHand.MAIN_HAND).is(Items.YELLOW_WOOL)) {
