...
public final class TestInputImpl implements TestInput {
	...
	public void holdKey(Function<Options, KeyMapping> keyBindingGetter) {
		...
		KeyMapping keyBinding = context.computeOnClient(client -> keyBindingGetter.apply(client.options));
		holdKey(keyBinding);
	}
	...
	public void holdControl() {
		...
		holdKey(Minecraft.ON_OSX ? InputConstants.KEY_LWIN : InputConstants.KEY_LCONTROL);
	}
	...
	public void releaseControl() {
		...
		releaseKey(Minecraft.ON_OSX ? InputConstants.KEY_LWIN : InputConstants.KEY_LCONTROL);
	}
	...
	private static void pressOrReleaseKey(Minecraft client, InputConstants.Key key, int action) {
		switch (key.getType()) {
		case KEYSYM -> client.keyboardHandler.keyPress(client.getWindow().getWindow(), key.getValue(), 0, action, 0);
		case SCANCODE -> client.keyboardHandler.keyPress(client.getWindow().getWindow(), GLFW.GLFW_KEY_UNKNOWN, key.getValue(), action, 0);
		case MOUSE -> ((MouseAccessor) client.mouseHandler).invokeOnPress(client.getWindow().getWindow(), key.getValue(), action, 0);
		}
	}
	...
	@Override
	public void holdKeyFor(Function<Options, KeyMapping> keyBindingGetter, int ticks) {
		ThreadingImpl.checkOnGametestThread("holdKeyFor");
		Preconditions.checkNotNull(keyBindingGetter, "keyBindingGetter");
		Preconditions.checkArgument(ticks >= 0, "ticks cannot be negative");
	...
	public void typeChar(int codePoint) {
		...
		context.runOnClient(client -> ((KeyboardAccessor) client.keyboardHandler).invokeCharTyped(client.getWindow().getWindow(), codePoint, 0));
	}
	...
	public void typeChars(String chars) {
		...
		context.runOnClient(client -> {
			chars.chars().forEach(codePoint -> {
				((KeyboardAccessor) client.keyboardHandler).invokeCharTyped(client.getWindow().getWindow(), codePoint, 0);
			});
		});
	}
	...
	public void scroll(double xAmount, double yAmount) {
		...
		context.runOnClient(client -> ((MouseAccessor) client.mouseHandler).invokeOnScroll(client.getWindow().getWindow(), xAmount, yAmount));
	}
	...
	public void setCursorPos(double x, double y) {
		...
		context.runOnClient(client -> ((MouseAccessor) client.mouseHandler).invokeOnMove(client.getWindow().getWindow(), x, y));
	}
	...
	public void moveCursor(double deltaX, double deltaY) {
		...
		context.runOnClient(client -> {
			...
			((MouseAccessor) client.mouseHandler).invokeOnMove(client.getWindow().getWindow(), newX, newY);
		});
	}
